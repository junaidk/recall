...
host:
	...
	CGO_ENABLED=1 go build -ldflags="$(LDFLAGS)" -o $(TARGET_DIR)/host/$(BINARY) $(PKG_SERVER)
	CGO_ENABLED=1 go build -ldflags="$(LDFLAGS)" -o $(TARGET_DIR)/host/$(SEED_BIN) $(PKG_SEED)
...
linux-amd64:
	...
	CGO_ENABLED=1 GOOS=linux GOARCH=amd64 \
	  ...
	  go build -ldflags="$(LDFLAGS)" -o $(TARGET_DIR)/linux-amd64/$(BINARY) $(PKG_SERVER)
...
linux-arm64:
	...
	CGO_ENABLED=1 GOOS=linux GOARCH=arm64 \
	  ...
	  go build -ldflags="$(LDFLAGS)" -o $(TARGET_DIR)/linux-arm64/$(BINARY) $(PKG_SERVER)
...
linux-armv7:
	...
	CGO_ENABLED=1 GOOS=linux GOARCH=arm GOARM=7 \
	  ...
	  go build -ldflags="$(LDFLAGS)" -o $(TARGET_DIR)/linux-armv7/$(BINARY) $(PKG_SERVER)
...
linux-armv6:
	...
	CGO_ENABLED=1 GOOS=linux GOARCH=arm GOARM=6 \
	  CC="$(ZIG) cc -target arm-linux-musleabihf" \
	  CXX="$(ZIG) c++ -target arm-linux-musleabihf" \
	  go build -ldflags="$(LDFLAGS)" -o $(TARGET_DIR)/linux-armv6/$(BINARY) $(PKG_SERVER)
